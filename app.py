...
# --- Skills ---
def arthi_greeting(user_input):
    greetings = ["hello", "hi", "hey", "good morning", "good evening"]
    if any(word in user_input for word in greetings):
        return "Hey there 👋 I'm arthi!"
    elif "how are you" in user_input:
        return "I'm running smoothly 😄 How about you?"
    return None    
    
def arthi_goodbye(user_input):
...
# --- open file ---
def arthi_open_file(user_input):
...
# --- Search Desktop DB ---
def arthi_search_desktop_db(user_input):
...
def arthi_access_db(user_input):
...
def arthi_system_info(user_input):
...
def arthi_play_song(user_input):
    ...
    if user_input.lower().startswith("play"):
        ...
        if song:
            ...
            if is_online():
                ...
                folder = r"C:\Users\JOHN MWAKIA\OneDrive\One Drive\OneDrive\Desktop\mini_arthi_web\Music"  # <-- Change to your music folder
...
def arthi_wikipedia_search(query):
    try:
...
def arthi_fact(user_input):
    # 🔹 Default fact lookup chain
    wiki_result = arthi_wikipedia_search(user_input)
    if wiki_result:
        return wiki_result
        
    google_result = arthi_google_search(user_input)
    if google_result:
...
def arthi_response(user_input):
    user_input = user_input.lower()

    # 1️⃣ Check greeting / basic commands first
    for func in [arthi_greeting, arthi_goodbye, arthi_time, arthi_date, arthi_open_file, arthi_play_song, is_online, arthi_fact, arthi_google_search, arthi_wikipedia_search, arthi_access_db, arthi_access_db, arthi_search_desktop_db]:
...
def chat():
    ...
    reply = arthi_response(user_message)
